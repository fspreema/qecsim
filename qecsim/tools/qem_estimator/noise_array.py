...
def return_noise_pos(circuit : stim.Circuit) -> np.array:
    ...
    -> In Monte Carlo we go to each of those locations and choose the channel depending 
    on the weight (or rather prob) of each channel
    *IMPORTANT*
    -> P_err is the prob for any error happening!
    -> Choose random between XYZ or IX, IY etc. in the different channels!
    """
    ...
    for _tick, inst in enumerate(circuit):
