...
__all__ = ["switched_circ"]

def switched_circ(*, lct : Context, patches: dict[str, Patch], 
    ...
    if before_round_depol > 0:
        switched_round_circ.append("DEPOLARIZE1", data, before_round_depol)
    ...
    if after_c_depol_prob > 0:
        switched_round_circ.append("DEPOLARIZE1", x_stab_index, after_c_depol_prob)
    ...
    for coord_pairs, order in stab_to_data.items():
        ...
        if order == "1-CX":
            ...
            switched_round_circ.append("CX", index_pairs)
    ...
    if after_c_depol_prob > 0:
        ...
        #Parallel Implementation of CX
            if order == "1-CX":
                ...
                index_pairs.append(q2i[coord_pairs[1]])
                index_pairs.append(q2i[coord_pairs[0]])
                switched_round_circ.append("DEPOLARIZE2", index_pairs, after_c_depol_prob)
    ...
    if before_m_flip_prob > 0:
        switched_round_circ.append("X_ERROR", x_stab_index + z_stab_index, before_m_flip_prob)
    ...
    if after_r_flip > 0:
        switched_round_circ.append("X_ERROR", x_stab_index + z_stab_index, after_r_flip)
